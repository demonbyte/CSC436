...
const Product = class Product {
    constructor(inputString) {
        const [name, price, availability] = inputString.split(",");
        this.name = name;
    ...
    static halfOff = (products) => {
        const newarr = [];
        products.map(product => newarr.push(new Product(`${product.name}',${product.price * .5},'${product.availability}`)))
        return newarr;
    }
    ...
    static printProducts = (products) =>  {
        for (let product of products) {
            if (product.availability === "In Stock") {
                console.log(`Product: ${product.name}, Cost: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(product.price)}, Availability: Yes`);
            }
            else {
                console.log(`Product: ${product.name}, Cost: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(product.price)}, Availability: No`);
            }
